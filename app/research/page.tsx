...
export default function Research() {
  return <>
    <header className="border-b border-[#b9c3bd] bg-[#dce1dc] py-20"><div className="page-wrap"><p className="eyebrow">Research papers / 03</p><h1 className="display mt-4 text-5xl font-extrabold sm:text-7xl">Questions worth measuring.</h1><p className="story-lead mt-6 max-w-3xl text-lg">These school research papers gave me room to design an experiment, build a model, challenge a first assumption, and explain what the evidence can—and cannot—say. PDFs are available for closer reading; the chemistry entry is identified as a working draft.</p></div></header>
    <section className="page-wrap story-section grid gap-5">
      {papers.map((paper, i) => <article key={paper.pdf} className="story-card grid gap-8 p-7 md:grid-cols-[.27fr_.73fr] md:p-9"><div><p className="technical text-xs text-[#a74727]">{String(i + 1).padStart(2, "0")} / {paper.field.toUpperCase()}</p><a href={paper.pdf} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex border-b-2 border-[#a74727] pb-1 text-sm font-extrabold text-[#a74727] hover:text-[#172832]">{paper.pdf.includes("draft") ? "Read the draft PDF" : "Read the full PDF"} &rarr;</a></div><div><h2 className="display text-2xl font-extrabold sm:text-3xl">{paper.title}</h2><div className="mt-6 grid gap-5 sm:grid-cols-3"><div><h3 className="eyebrow">The question</h3><p className="mt-2 text-sm leading-7 text-[#465960]">{paper.question}</p></div><div><h3 className="eyebrow">What I found</h3><p className="mt-2 text-sm leading-7 text-[#465960]">{paper.result}</p></div><div><h3 className="eyebrow">The limitation</h3><p className="mt-2 text-sm leading-7 text-[#465960]">{paper.setback}</p></div></div></div></article>)}
    </section>
  </>;
}

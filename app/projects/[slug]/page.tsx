    ...
    <>
      <header className="border-b border-[#d9ded8] bg-[#e9ece7]">
        <div className="page-wrap py-10 sm:py-16">
          ...
          <div className="mt-10 flex flex-wrap gap-3 border-t border-[#cbd2cc] pt-5 text-[11px] font-bold uppercase tracking-[.13em] text-[#59676b]"><span className="pr-3 text-[#a74727]">{project.status.replaceAll("-", " ")}</span>{project.skills?.map((skill) => <span key={skill} className="border-l border-[#cbd2cc] pl-3">{skill}</span>)}</div>
        </div>
      </header>

      {project.coverImage && <div className="page-wrap py-8 sm:py-12"><div className="paper-grid relative aspect-[16/8] overflow-hidden border border-[#d9ded8] bg-[#dde3df]"><Image src={project.coverImage} alt={`${project.title} design render`} fill priority sizes="(max-width: 1180px) 100vw, 1180px" className="object-cover" /></div><p className="technical mt-3 text-[10px] uppercase text-[#68757a]">Project render / {project.title}</p></div>}

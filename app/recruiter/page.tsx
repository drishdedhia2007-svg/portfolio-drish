...
export default function Recruiter() {
  ...
  return (
    <>
      <header className="blueprint-grid bg-[#192a34] py-20 text-[#fffefa] sm:py-26"><div className="page-wrap"><p className="technical text-[11px] uppercase text-[#d99d79]">Quick view / for recruiters</p><h1 className="display mt-4 max-w-4xl text-5xl font-extrabold leading-[1.05] sm:text-7xl">Engineering potential,<br /><span className="text-[#d99d79]">shown through work.</span></h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[#c4d0d0]">I am Drish Dedhia, a mechanical engineering student at RWTH Aachen. I enjoy making ideas tangible, asking what could fail, and improving the next version. Here is a short route through the work behind that approach.</p><div className="mt-9 flex flex-wrap gap-3"><Link href="/projects" className="button-primary">View case studies <span aria-hidden="true">&rarr;</span></Link><Link href="/about" className="button-light">About me</Link></div></div></header>
      ...
      <section className="border-y border-[#35515b] bg-[#142832] py-18"><div className="page-wrap grid gap-10 lg:grid-cols-[.32fr_.68fr]"><div><p className="eyebrow">Project evidence</p><h2 className="display mt-3 text-3xl font-extrabold">See the decisions.</h2><p className="mt-4 text-sm leading-7 text-[#c0cfca]">I show the brief, the decisions, the mistakes, and what I would change. A model is more useful when you can see the thinking behind it.</p></div><div className="grid gap-6 sm:grid-cols-2">{featured.map((project, index) => <ProjectCard key={project.slug} project={project} index={index + 1} />)}{featured.length === 0 && <p className="text-[#c0cfca]">Project case studies are being prepared.</p>}</div></div></section>
